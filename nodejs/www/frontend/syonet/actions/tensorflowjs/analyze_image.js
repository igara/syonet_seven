// @flow
import { save, load } from '../../tensorflows/image'
...
export default class AnalyzeImageAction {
	...
	onInputModelName(event: SyntheticInputEvent<HTMLInputElement>) {
		...
		this.Stores.AnalyzeImageStore.ModelName(name)
	}
	...
	onClickCategoryName(id: number) {
		this.Stores.AnalyzeImageStore.SelectedCategoryID(id)
	}
	...
	async onInputCategoryFile(
		...
		}> = this.Stores.AnalyzeImageStore.Category().reduce(
			(accumulator, currentValue) => {
				if (id === currentValue.id) {
					if (
		...
		this.Stores.AnalyzeImageStore.Category(category)
		console.log(this.Stores.AnalyzeImageStore.Category())
		m.redraw()
	}
	...
	onClickRemoveImage(id: number, index: number) {
		const category = this.Stores.AnalyzeImageStore.Category().map(category => {
			if (id === category.id) {
				return {
					...category,
