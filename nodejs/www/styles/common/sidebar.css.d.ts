declare const styles: {
  readonly "sidebar_exit": string;
  readonly "sidebar_exit_slider": string;
  readonly "sidebar_wrap_div": string;
  readonly "sidebar_overlay_div": string;
  readonly "hidden": string;
  readonly "sidebar_overlay_div_slider_hidden": string;
  readonly "able": string;
  readonly "sidebar_overlay_div_slider_able": string;
  readonly "sidebar_link_wrap_ul": string;
  readonly "sidebar_link_wrap_ul_slider_hidden": string;
  readonly "sidebar_link_wrap_ul_slider_able": string;
  readonly "sidebar_link_list": string;
};
export = styles;

